...
int oam_cli_mep_info_display(struct oamsim_cli_msg *msg) {
	...
	CLI(
			"MEP INTF: %d  MEPID: %d  MD_NAME: %s  MD_LEVEL: %d  MA_NAME: %s  ASSOC_ID: %d  INTERVAL: %d  TX_COUNT: %u\n",
			mep->tx_mep->mep_if, mep->mep_id, mep->md->md_name,
			mep->md->md_level, mep->ma->ma_name, mep->assoc_id,
			mep->cc.tx_interval, mep->cc.tx_count);
...
int oam_cli_print_all_meps(void) {
	...
	while (md != NULL) {
		...
		while (ma != NULL) {
			...
			while (mep != NULL) {
				CLI(
						"MEP_INTF: %d  MEPID: %d ASSOC_ID: %d INTERVAL: %d TX_COUNT: %u\n",
						mep->tx_mep->mep_if, mep->mep_id, mep->assoc_id,
						mep->cc.tx_interval, mep->cc.tx_count);
				mep = mep->list.nxt;
			}
	...
	for (tlv = mep->tlv_list; tlv != NULL; tlv = tlv->list.nxt) {
		if (tlv->tlv_type == 2 || tlv->tlv_type == 4)
			CLI("TLV_TYPE: %d TLV_SIZE: %d TLV_VALUE: %x\n", tlv->tlv_type,
					tlv->tlv_size, tlv->tlv_val[0]);
		else
			CLI("TLV_TYPE: %d TLV_SIZE: %d TLV_VALUE: %s\n", tlv->tlv_type,
					tlv->tlv_size, tlv->tlv_val);
	}
	...
	if (strcmp(cmd, "config") == 0) {
		...
		if (strcmp(cmd, "mep") == 0) {
			...
			if (strcmp(cmd, "mepid") == 0) {
				...
				mep_num = parse_cli_cmd_and_create_mep(&oam_msg);
				CLI("MEP interface: %d\n",mep_num);
						...
						if (strcmp(cmd, "val") == 0) {
							...
							if ((oam_msg.tlv_type == 2)
								...
								if (strlen(&oam_msg.tlv_val) > 40) {
									LOG(
											"TLV value length is to long. Maximum supported TLV length is 40 Bytes\n");
									CLI("%s\n", CLI_STATUS(-1));
									return;
								}
							}
...
void socket_thread_handler(void *ptr) {
	int sockfd, err;
	...
	if ((err = bind(sockfd, &addr, sizeof(addr))) < 0) {
		perror ("\nCould not bind to addr and port\n");
		pthread_exit(-1);
	}
	getsockname(sockfd, &addr, sizeof(addr));
	...
	while(is_py_running) {
		...
		if (sendto(sockfd, buffer, sizeof(buffer), 0, &cliaddr, cli_addr_len) < 0) {
			perror("sendto()");
		}
		printf ("\nSent response: %s\n",buffer);
	}
...
int oamsim_remote_msg_init() {
	is_py_running = TRUE;
	return pthread_create(&py_thread, NULL, socket_thread_handler, NULL);
}
